...
def load_pbp_data(year: int):
    ...
    if type(year) is not int:
        raise TypeError(
            "Please provide an integer between 1999 and 2024 for the year argument."
        )

    if year < 1999 or year > 2024:
        raise SeasonNotFoundError(
            "Play by play data is only available from 1999 to 2024."
        )
...
def _validate_year_range(years: list[int]) -> None:
